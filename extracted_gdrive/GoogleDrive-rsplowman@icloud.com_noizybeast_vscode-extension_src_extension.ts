    ...
    gabrielWs.onmessage = (e: MessageEvent) => {
      try {
        ...
        if (msg.type === "empire.status") {
          log(
            `Empire: actors=${msg.actors} tokens=${msg.active_tokens} ledger=${msg.ledger_events}`,
          );
        }
...
export function activate(ctx: vscode.ExtensionContext) {
  ...
  reg(ctx, "noizybeast.turboPipeline", () =>
    runTurboScript("turbo_pipeline.sh", "Pipeline: HEAL→DEDUPE→UNIFY→VERIFY"),
  );
  reg(ctx, "noizybeast.turboZap", () =>
    runTurboScript("turbo_zap.sh", "Zap: DNS flush + network reset"),
  );
  reg(ctx, "noizybeast.turboGitSync", () =>
    runTurboScript("turbo_git_sync.sh", "Git Sync: pushing all repos"),
  );
  reg(ctx, "noizybeast.turboReset", () =>
    runTurboScript("turbo_reset.sh", "Reset: full environment reset"),
  );
  reg(ctx, "noizybeast.turboMountOmen", () =>
    runTurboScript("turbo_mount_omen.sh", "Mount: HP-OMEN volume"),
  );
  ...
  vscode.window
    .showInformationMessage(
      "⚡ NOIZYBEAST activated — GABRIEL V4 online",
      "Open Empire Panel",
      "T2 Flow Sync",
    )
    .then((choice) => {
...
async function cmdT3() {
  ...
  const worker =
    (await vscode.window.showQuickPick(workers, {
...
async function cmdT11() {
  const url = `${cfg<string>("n8nUrl") || "http://localhost:5678"}/webhook/reconnect-fast`;
  const operator = cfg<string>("operator");
  ...
  try {
    ...
    outputChannel.appendLine(
      `   gabriel: ${s.gabriel_daemon?.alive ? "ALIVE" : "DOWN"} (${s.gabriel_daemon?.bosses ?? 0} bosses)`,
    );
    ...
    vscode.window.showErrorMessage(
      `T11 RECONNECT FAST failed — is n8n running on :5678? (${e.message || e})`,
    );
  }
}

// ── T12 RECONNECT SMART · 5–10min full healing + classification ──
async function cmdT12(ctx: vscode.ExtensionContext) {
  const url = `${cfg<string>("n8nUrl") || "http://localhost:5678"}/webhook/reconnect-smart`;
  const operator = cfg<string>("operator");
  outputChannel.show(true);
  ...
  vscode.window.showInformationMessage(
    "🔄 T12 RECONNECT SMART starting · ~5–10 min · digest panel will open when complete",
  );
  statusBar.flash("T12");
  ...
  try {
    ...
    const digest = (await res.json()) as any;
    ...
    vscode.window.showErrorMessage(
      `T12 RECONNECT SMART failed — ${e.message || e} · check n8n logs`,
    );
  }
}

function showT12DigestPanel(digest: any) {
  const panel = vscode.window.createWebviewPanel(
    "noizyT12Digest",
    "T12 RECONNECT SMART — Healing Digest",
  ...
  const woundsHtml = wounds.length
    ? wounds
        .map(
          (w: any) =>
            `<li><span class="chip ${safe(w.severity || "info")}">${safe(w.severity || "?")}</span> ${safe(w.title || w.message || "")} <span class="owner">→ ${safe(w.owner || "unassigned")}</span></li>`,
        )
        .join("")
    : '<li class="muted">No wounds surfaced this pass.</li>';
  const findingsHtml = findings.length
    ? findings
        .map((f: any) => `<li>${safe(f.path || f.kind || "")} — ${safe(f.note || "")}</li>`)
        .join("")
...
async function cmdActivateVoice() {
  ...
  terminal.show();
  terminal.sendText(
    `rec "${recPath}" rate 16k channels 1 silence 1 0.1 1% 1 2.0 3% trim 0 15 2>/dev/null && echo "VOICE_DONE"`,
  );
...
async function cmdLucyAsk() {
  ...
  try {
    ...
    if (data.response) {
      ...
      vscode.window.showInformationMessage(
        `LUCY: ${data.response.slice(0, 100)}${data.response.length > 100 ? "..." : ""}`,
      );
    } else {
      outputChannel.appendLine(`Error: ${data.error}`);
    }
...
function getCfg() {
  ...
  return {
    ...
    n8nUrl: c.get<string>("n8nUrl", "http://localhost:5678"),
  };
}

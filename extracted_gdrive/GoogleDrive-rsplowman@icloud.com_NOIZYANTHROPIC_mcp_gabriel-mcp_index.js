...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    {
      name: "gabriel_continue_status",
      description: "Check if the Continue configuration files (.json and .yaml) contain the local GABRIEL OpenAI endpoint",
      inputSchema: { type: "object", properties: {} },
    },
    {
      name: "gabriel_continue_configure",
      description: "Automatically configure Continue (.json and .yaml) to use local GABRIEL Daemon as an LLM provider",
      inputSchema: { type: "object", properties: {} },
    },
  ],
}));
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "gabriel_continue_status": {
        const continueDir = join(homedir(), ".continue");
        const configJsonPath = join(continueDir, "config.json");
        const configYamlPath = join(continueDir, "config.yaml");
        ...
        if (existsSync(configJsonPath)) {
          try {
            const raw = readFileSync(configJsonPath, "utf8");
            const parsed = JSON.parse(raw);
            const models = parsed.models || [];
            jsonConfigured = models.some(
              (m) =>
                m.apiBase &&
                (m.apiBase.includes("http://127.0.0.1:9777") ||
                  m.apiBase.includes("http://localhost:9777")),
            );
            details.push(`config.json exists (configured: ${jsonConfigured})`);
          } catch (err) {
            details.push(`config.json error: ${err.message}`);
          }
        } else {
          details.push("config.json not found");
        }

        if (existsSync(configYamlPath)) {
          try {
            const raw = readFileSync(configYamlPath, "utf8");
            yamlConfigured =
              raw.includes("http://127.0.0.1:9777") ||
              raw.includes("http://localhost:9777");
            details.push(`config.yaml exists (configured: ${yamlConfigured})`);
          } catch (err) {
            details.push(`config.yaml error: ${err.message}`);
          }
        } else {
          details.push("config.yaml not found");
        }
          ...
          content: [
            {
              type: "text",
              text: [
                `**GABRIEL CONTINUE STATUS**`,
                `JSON Configured: ${jsonConfigured ? "YES ✓" : "NO ✗"}`,
                `YAML Configured: ${yamlConfigured ? "YES ✓" : "NO ✗"}`,
                ``,
                `Details:`,
                ...details.map((d) => `  - ${d}`),
              ].join("\n"),
      ...
      case "gabriel_continue_configure": {
        const continueDir = join(homedir(), ".continue");
        const configJsonPath = join(continueDir, "config.json");
        const configYamlPath = join(continueDir, "config.yaml");
        let results = [];
        ...
        if (existsSync(configJsonPath)) {
          try {
            ...
            const exists = parsed.models.some(
              (m) =>
                m.apiBase &&
                (m.apiBase.includes("http://127.0.0.1:9777") ||
                  m.apiBase.includes("http://localhost:9777")),
            ...
            if (!exists) {
              parsed.models.push({
                title: "GABRIEL (FOSS Daemon)",
                provider: "openai",
                model: "gabriel",
                apiBase: "http://127.0.0.1:9777/v1",
              });
              writeFileSync(configJsonPath, JSON.stringify(parsed, null, 2));
              results.push("config.json: Added GABRIEL (FOSS Daemon) to models.");
            } else {
              results.push("config.json: Already contains GABRIEL configuration.");
            }
          } catch (err) {
            results.push(`config.json error: ${err.message}`);
          }
        } else {
          results.push("config.json not found");
        }
        ...
        if (existsSync(configYamlPath)) {
          try {
            ...
            const exists =
              ...
              raw.includes("http://localhost:9777");
            if (!exists) {
              // Find the models section and insert the GABRIEL config
              const lines = raw.split("\n");
              const modelsIndex = lines.findIndex((l) => l.trim().startsWith("models:"));
              if (modelsIndex !== -1) {
                // Insert right after the "models:" line
                const block = [
                  "  - name: GABRIEL (FOSS Daemon)",
                  "    provider: openai",
                  "    model: gabriel",
                  "    apiBase: http://127.0.0.1:9777/v1",
                  "    roles: [chat, edit]",
                ];
                lines.splice(modelsIndex + 1, 0, ...block);
                writeFileSync(configYamlPath, lines.join("\n"));
                results.push("config.yaml: Added GABRIEL (FOSS Daemon) to models.");
              } else {
                results.push("config.yaml error: 'models:' section not found.");
              }
            } else {
              results.push("config.yaml: Already contains GABRIEL configuration.");
            }
          } catch (err) {
            results.push(`config.yaml error: ${err.message}`);
          }
        } else {
          results.push("config.yaml not found");
        }
          ...
          content: [
            {
              type: "text",
              text: [
                `**GABRIEL CONTINUE CONFIGURATION**`,
                ``,
                `Results:`,
                ...results.map((r) => `  - ${r}`),
              ].join("\n"),

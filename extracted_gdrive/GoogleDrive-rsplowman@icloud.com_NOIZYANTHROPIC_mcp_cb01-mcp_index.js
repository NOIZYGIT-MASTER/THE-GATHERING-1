 ...
 *   cb01_domain_audit      — Live NS audit across all NOIZY domains (2026-04-20 upgrade)
 *   cb01_domain_flip       — Flip NS for one domain (GoDaddy API → CF Transfer-In fallback)
 *
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description:
        "Live NS audit across all NOIZY domains. Uses dig only — no auth needed. Returns which domains are GREEN (NS at marek/tara = NOIZYFISH) vs PENDING.",
      inputSchema: { type: "object", properties: {} },
    ...
    switch (name) {
      ...
      case "cb01_smoke_test": {
        ...
        if (!existsSync(smokeTestPath)) {
          return {
            content: [{ type: "text", text: "⚠ smoke_test.sh not found at project root" }],
          };
        }
      ...
      case "cb01_domain_audit": {
        ...
        let output;
        try {
          // execFileSync with argv array — no shell, no injection risk.
      ...
      case "cb01_domain_flip": {
        ...
        // Strict allowlist — prevents flipping anything outside the empire.

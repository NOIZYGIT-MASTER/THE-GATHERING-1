...
do_kill() {
    ...
    killall -9 "YouTube" 2>/dev/null || true
...
do_migrate_media() {
    ...
    for user in "${MICKYP_USERS[@]}"; do
        ...
        find "$home" -not -path "*/Library/*" \
            ...
            local rel=$(dirname "${f#$home/}")
            local base=$(basename "$f")
            mkdir -p "$dest/audio/$rel"
            local h1=$(md5 -q "$f" 2>/dev/null)
            rsync -a "$f" "$dest/audio/$rel/$base"
            local h2=$(md5 -q "$dest/audio/$rel/$base" 2>/dev/null)
            [ "$h1" = "$h2" ] && rm "$f" && log "  ✓ $base" || log "  ✗ MISMATCH: $base — kept"
        done
        ...
        find "$home" -not -path "*/Library/*" \
            ...
            local h1=$(md5 -q "$f" 2>/dev/null)
            rsync -a "$f" "$dest/video/$rel/$base"
            local h2=$(md5 -q "$dest/video/$rel/$base" 2>/dev/null)
            [ "$h1" = "$h2" ] && rm "$f" && log "  ✓ $base" || log "  ✗ MISMATCH: $base — kept"
        done
            ...
            ls -1 "$full" 2>/dev/null | while IFS= read -r plugin; do
                ...
                if [ "$skip" = false ]; then
                    ...
                    if [ "$DRY_RUN" = false ]; then
                        rsync -az --remove-source-files \
                            "${full}/${plugin}" \
                            "${GOD_USER}@${GOD_IP}:/Library/Audio/Plug-Ins/$(basename $(dirname "$full"))/${plugin}" 2>> "$REPORT"
                    fi
                fi
            done
        done
...
const http=require('http'),{execSync}=require('child_process'),fs=require('fs');
const PORT=7006,AGENT='ENGR_KEITH',SCRIPT=`${process.env.HOME}/Desktop/noizy_logic_control.applescript`;
const LOG=`${process.env.HOME}/Desktop/noizy_session_log.txt`;
const state={agent:AGENT,status:'ONLINE',session:'NOIZY Claude Session 1',recording:false,armed:false,boot:new Date().toISOString()};
function log(m){const l=`[${new Date().toISOString()}] [${AGENT}] ${m}`;console.log(l);fs.appendFileSync(LOG,l+'\n')}
function run(cmd){try{return{ok:true,r:execSync(`osascript ${SCRIPT} "${cmd}"`,{encoding:'utf-8',timeout:10000}).trim()}}catch(e){return{ok:false,e:e.message}}}
function handle(cmd){log(`CMD: ${cmd}`);state.lastCmd=cmd;state.lastTime=new Date().toISOString();
switch(cmd){case'record':const rr=run('record');if(rr.ok){state.recording=true;state.armed=true;state.status='RECORDING'}return{...state,action:rr};
case'stop':const sr=run('stop');if(sr.ok){state.recording=false;state.status='STOPPED'}return{...state,action:sr};
case'arm':const ar=run('arm');if(ar.ok){state.armed=true;state.status='ARMED'}return{...state,action:ar};
case'save':run('save');return{...state,action:{ok:true}};
case'play':run('play');state.status='PLAYING';return state;
case'status':return state;case'ping':return{agent:AGENT,alive:true};
default:return{error:`Unknown: ${cmd}`}}}
http.createServer((req,res)=>{res.setHeader('Content-Type','application/json');
if(req.method==='GET'&&req.url==='/status'){res.end(JSON.stringify(state,null,2));return}
if(req.method==='GET'&&req.url==='/ping'){res.end(JSON.stringify({agent:AGENT,alive:true}));return}
if(req.method==='POST'&&req.url==='/dispatch'){let b='';req.on('data',c=>b+=c);req.on('end',()=>{try{const p=JSON.parse(b);res.end(JSON.stringify(handle((p.command||p.cmd||'').toLowerCase()),null,2))}catch(e){res.statusCode=400;res.end(JSON.stringify({error:e.message}))}});return}
const q=req.url.replace('/','').toLowerCase();if(['record','stop','arm','save','play'].includes(q)){res.end(JSON.stringify(handle(q),null,2));return}
res.statusCode=404;res.end(JSON.stringify({endpoints:['GET /record','GET /stop','GET /arm','GET /save','GET /play','GET /status','GET /ping','POST /dispatch']}))
}).listen(PORT,'0.0.0.0',()=>{log(`ENGR_KEITH ONLINE — port ${PORT}`);log('Waiting for RSP_001...')});
KEITHJS
...
NOIZYCLI
    ...
    # Start KEITH
    ssh "${GOD_USER}@${GOD_IP}" "nohup node ~/Desktop/engr_keith_server.js > ~/Desktop/engr_keith.log 2>&1 &"
...
case "${1:-all}" in
    ...
    report)     do_report ;;
    *)
        echo "NOIZY MASTER BUILD"
        echo "Usage: $0 {all|kill|audit|media|plugins|nuke|god|keith|aunet|ram|license|report}"
        echo ""
